        ...
        while currentMetrics["f1score"] > previousMetrics["f1score"]:
            ...
            if arguments.algorithm == "hmm":
                if arguments.hmmtrainwith == "malware":
                    allJSONFiles = Trace.loadJSONTraces(glob.glob("%s/*.json" % (arguments.malwaredir, arguments.fileextension)), "malware")
                    allJSONFiles += Trace.loadJSONTraces(glob.glob("%s/*.json" % (arguments.goodwaredir, arguments.fileextension)), "goodware")
                else:
                    allJSONFiles = Trace.loadJSONTraces(glob.glob("%s/*.json" % (arguments.goodwaredir, arguments.fileextension)), "malware")
                    allJSONFiles += Trace.loadJSONTraces(glob.glob("%s/*.json" % (arguments.malwaredir, arguments.fileextension)), "goodware")
