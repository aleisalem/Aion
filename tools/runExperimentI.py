            ...
            if arguments.analyzeapks == "yes":
                ...
                for app in allApps:
                    if arguments.analysisengine == "droidutan":
                        ...
                        # 2.a. Get a handle
                        jsonTraceFile = open(inFile.replace(".db", ".json"), "w")
                        # 7.b. Write content
                        jsonTraceFile.write(jsonTrace)
                        jsonTraceFile.close()

                    elif arguments.analysisengine == "droidbot":
                        # 0. Retrieve the filtered logcat file
                        inFile = app.replace(".apk", "_droidbot/logcat_filtered.log")
                        # 1. Check its existence
                        if not os.path.exists(inFile):
                            prettyPrint("Unable to find filtered log file for app: \"%s\"" % app, "warning")
                            continue
